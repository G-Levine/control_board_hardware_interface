...
void spine_to_spi(spi_data_t *data, spine_data_t *spine_data, int leg_0) {
  ...
  // printf("SPI DATA (%i): ", leg_0 / 2);
  // for (int j = 0; j < 15; j++) {
  //   printf("\t%i 0x%hx", j, ((uint32_t *)spine_data)[j]);
  // }
  // printf("\n");
}
...
void spi_send_receive(spi_command_t *command, spi_data_t *data) {
  ...
  for (int spi_board = 0; spi_board < 2; spi_board++) {
    ...
    // printf("SPI RX BUFFER BEFORE: ");
    // for (int j = 0; j < 15; j++) {
    //   printf("\t0x%hx", ((uint32_t *)rx_buf)[j]);
    // }
    // printf("\n");
    ...
    // for (int j = 0; j < 15; j++) {
    //   printf("\t0x%hx", ((uint32_t *)data_d)[j]);
    // }
    // printf("\n");
